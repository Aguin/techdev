...
//        word2LinearLayout.setOnTouchListener(new TouchListener());
    ...
    public boolean onStartGame(View view) {
        ...
        int sz = words.size();
        word1 = words.get(random.nextInt(sz));
        word2 = words.get(random.nextInt(sz));
        String scr = "";
        int len1 = word1.length(), len2 = word2.length();
        int cnt1 = 0, cnt2 = 0;
        while(cnt1 < len1 || cnt2 < len2) {
            int k = random.nextInt(len1 - cnt1 + len2 - cnt2);
            if (k < len1 - cnt1) {
                scr += word1.charAt(cnt1);
                cnt1++;
            }
            else {
                scr += word2.charAt(cnt2);
                cnt2++;
            }
        }
        messageBox.setText(scr);
        int len = scr.length();
        for (int i = len - 1; i >= 0; --i) {
            LetterTile tile = new LetterTile(this, scr.charAt(i));
